...
namespace aalta
{
	...
	protected:
		...
		typedef hash_map<int, std::set<int>> coi_map;
		...
		std::set<int> coi_of_assumption();						// get COI for assumptions
		void coi_of(int id, std::set<int> &res);				// get COI for the given \@id, results are stored in \@ res
		void coi_merge(std::set<int> &to, std::set<int> &from); // merge the coi information from \@ from to \@ to
		...
		{
			return (clauses_added_.find(f) != clauses_added_.end());
		}
	...
	inline aalta_formula *Solver::formula_of(int id)
	{
		formula_map::iterator it = formula_map_.find(id); 
		return (it != formula_map_.end())? it->second : NULL;
	}

	inline void Solver::coi_merge(std::set<int> &to, std::set<int> &from)
	{
		std::set<int> tmp;
		std::set_union(from.begin(), from.end(), to.begin(), to.end(), std::inserter(tmp, tmp.begin()));
		to.swap(tmp);
	}
