 ...
 */
 ...
 namespace aalta
 {
 		...
 		switch (f->oper ())
 		{
 			case aalta_formula::Not:  //id -> id for Literals
 				if (f->r_af () != NULL)
 				{
 					...
 					coi_map_.insert (std::pair<int, std::set<int> >(id, v));
 					ids.push_back (id);
 					break;
 				}
	...
	{
		std::set<int> coi = coi_of_assumption ();
		//std::vector<int> coi_2 = coi_of_global ();
		//coi_merge (coi, coi_2);
		size_t sz=assign.size ();
		for (size_t i = 0; i < sz; ++i)
		{
			// if (i < coi.size ())
			// {
				if (coi.find(i) == coi.end()) //is not a coi
					assign [i] = 0; 
			// }
			// else
			// 	assign [i] = 0;
	...
	std::set<int> Solver::coi_of_assumption ()
	{
		std::set<int> res;
		// res.resize (nVars (), 0);
		for (int i = 0; i < assumption_.size (); ++ i)
	...
	void Solver::coi_of (int id, std::set<int>& res)
	{
		int abs_id = abs(id);
		coi_map::iterator it = coi_map_.find (abs_id);
		if (it != coi_map_.end ())
			coi_merge (res, it->second);
		else //check whether id represent a literal or Next
		{
			// assert (res.size () >= abs (id));
			// res.erase(abs(id)-1);// [] = 0;
			aalta_formula* f = formula_of (id);
			if (f != NULL && (f->oper () > aalta_formula::Undefined 
							|| f->oper () == aalta_formula::Not 
							|| f->oper () == aalta_formula::Next))
			{
				//COI includes only atoms
				res.insert(abs_id-1);//[] = 1;
			}
			else
				res.erase(abs_id-1);// [] = 0;
		}
	}
